...
class ControllerTPP:
    """Controller implementing the thermal power plant schedule described in the module docstring."""
    ...
    def __init__(self, h_dict):
        ...
        self.unit_capacities = [h_dict[self.component_name][unit_name]["rated_capacity"] 
    ...
    def step(self, h_dict):
        ...
        # If the power setpoint fraction is provided as a list for each unit, use it directly. Otherwise, assume it's a fraction of the 
        # total rated capacity and distribute it proportionally to the unit capacities.
        if isinstance(h_dict["plant"]["power_setpoint_schedule"]["power_setpoint_fraction"][time_index], (list, tuple)):
            power_setpoint_fraction = h_dict["plant"]["power_setpoint_schedule"]["power_setpoint_fraction"][time_index]
            power_setpoints = [power_setpoint * unit_capacity
                for power_setpoint, unit_capacity in 
                zip(power_setpoint_fraction, self.unit_capacities)
            ]
            h_dict[self.component_name]["power_setpoints"] = power_setpoints
        else:
            power_setpoint_fraction = h_dict["plant"]["power_setpoint_schedule"]["power_setpoint_fraction"][time_index]
            h_dict[self.component_name]["power_setpoint"] = power_setpoint_fraction * self.rated_capacity
