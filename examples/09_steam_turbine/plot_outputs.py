...
ax = axarr[0]
ax.plot(time_minutes, df["steam_turbine.power"] / 1000, label="Power Output", color="b")
ax.plot(
    time_minutes,
    df["steam_turbine.power_setpoint"] / 1000,
    label="Power Setpoint",
    color="r",
    linestyle="--",
)
ax.axhline(
    h_dict["steam_turbine"]["rated_capacity"] / 1000,
    color="gray",
    linestyle=":",
    label="Rated Capacity",
)
ax.axhline(
    h_dict["steam_turbine"]["min_stable_load_fraction"]
    * h_dict["steam_turbine"]["rated_capacity"]
    / 1000,
    color="gray",
    linestyle="--",
...
# Plot the state
ax = axarr[1]
ax.plot(time_minutes, df["steam_turbine.state"], label="State", color="k")
ax.set_ylabel("State")
