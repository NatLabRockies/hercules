...
fig, axarr = plt.subplots(5, 1, sharex=True, figsize=(10, 10))

# Plot the power output and setpoint
ax = axarr[0]
ax.plot(time_minutes, df["combined_cycle_gas_turbine.power"] / 1000, label="Power Output", color="b")
ax.plot(
    time_minutes,
...
ax.plot(
    time_minutes,
    df["combined_cycle_gas_turbine.time_in_state"],
    label="Time in State",
    color="purple",
)
ax.set_ylabel("Time [s]")
ax.set_title("Time in State")
ax.grid(True)
