...
def test_init_from_dict():
    ...
    st = SteamTurbine(copy.deepcopy(h_dict_steam_turbine), "steam_turbine")
...
def test_default_inputs():
    ...
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.ramp_rate_fraction == 0.04

    # Test that the run_up_rate_fraction input is correct from input dict
    assert st.run_up_rate_fraction == 0.02
    ...
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.run_up_rate_fraction == st.ramp_rate_fraction
    ...
    del h_dict["steam_turbine"]["run_up_rate_fraction"]
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.ramp_rate_fraction == 0.03
    assert st.run_up_rate_fraction == 0.03
    ...
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.min_stable_load_fraction == 0.30
    assert st.hot_startup_time == 7.5 * 60.0 * 60.0  # 7.5 hours in seconds
    assert st.warm_startup_time == 7.5 * 60.0 * 60.0  # 7.5 hours in seconds
    assert st.cold_startup_time == 7.5 * 60.0 * 60.0  # 7.5 hours in seconds

    h_dict = copy.deepcopy(h_dict_steam_turbine)
    del h_dict["steam_turbine"]["min_up_time"]
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.min_up_time == 48 * 60.0 * 60.0  # 48 hours in seconds

    h_dict = copy.deepcopy(h_dict_steam_turbine)
    del h_dict["steam_turbine"]["min_down_time"]
    st = SteamTurbine(h_dict, "steam_turbine")
    assert st.min_down_time == 48 * 60.0 * 60.0  # 48 hours in seconds
...
def test_default_efficiency_table():
    ...
    np.testing.assert_array_equal(
        st.efficiency_power_fraction,
        np.array([0.3, 0.5, 1.0], dtype=hercules_float_type),
    )
    np.testing.assert_array_equal(
        st.efficiency_values,
