...
``SolarPySAMPVWatts`` that runs PySAM once on the resource-resolution weather
...
def _build_synthetic_solar_file(path, starttime_utc, endtime_utc, dt_seconds):
    ...
    linear over each resource interval (this is what makes approach 2
    ...
    Args:
        ...
        dt_seconds (float): resource time-step of the synthetic file in seconds.
...
def test_resource_dt_matches_full_resolution_within_tolerance(tmp_path):
    ...
    resource-dt + upsample path is numerically equivalent to the existing
    ...
    _build_synthetic_solar_file(solar_input, starttime_utc, endtime_utc, dt_resource)

    h_dict_resource = _build_h_dict(
        solar_input, starttime_utc, endtime_utc, dt, use_resource_solar_dt=True
    )
    spv_resource = SolarPySAMPVWatts(h_dict_resource, "solar_farm")
    ...
    assert spv_resource._compute_dt == pytest.approx(dt_resource)
    assert spv_full._compute_dt == pytest.approx(dt)

    expected_n = int((spv_resource.endtime - spv_resource.starttime) / dt)
    assert spv_resource.ac_power_available_array.shape[0] == expected_n
    ...
    np.testing.assert_allclose(
        spv_resource.ac_power_available_array,
        spv_full.ac_power_available_array,
        rtol=1e-3,
        atol=10.0,
    )
    np.testing.assert_allclose(
        spv_resource.dc_power_available_array,
...
def test_resource_dt_is_no_op_when_dt_solar_equals_dt(tmp_path):
    """The fallback path should behave exactly like the pre-feature code.

    When the weather file's resource dt equals the Hercules dt, ``_compute_dt``
...
def test_use_resource_solar_dt_false_forces_fallback(tmp_path):
    """Explicit opt-out forces the existing path even on coarse-resource data.
    ...
    assert spv._compute_dt == pytest.approx(dt)
    assert spv.dt_solar == pytest.approx(dt_resource)


def test_resource_dt_uses_resource_timestamp_alignment_for_offset_start(tmp_path):
    """resource path should anchor compute stamps to weather-file boundaries.

    With an offset simulation start and coarse resource weather timestamps, the
    compute grid should use the resource stamp immediately before starttime and
    the resource stamp at/after endtime (rather than start + n * dt_resource).
    """
    ...
    solar_input = tmp_path / "solar_input.ftr"
    _build_synthetic_solar_file(solar_input, file_start_utc, sim_end_utc, dt_resource)
    ...
    assert spv_resource._compute_dt == pytest.approx(dt_resource)
    assert spv_resource._compute_time_steps[0] == pytest.approx(-30.0)
    assert spv_resource._compute_time_steps[-1] == pytest.approx(330.0)
    np.testing.assert_allclose(np.diff(spv_resource._compute_time_steps), dt_resource)


def test_resource_dt_includes_previous_stamp_when_start_aligns_to_resource(tmp_path):
    """Compute grid should include the resource stamp before an aligned start.

    When ``starttime_utc`` falls exactly on a resource weather timestamp and
    the file extends earlier, the compute grid must include that previous
    resource stamp so ``_upsample_outputs_to_hercules_dt`` has midpoints on
    ...
    assert spv._compute_dt == pytest.approx(dt_resource)
    assert spv._compute_time_steps[0] == pytest.approx(-dt_resource)
    assert spv._compute_time_steps[1] == pytest.approx(0.0)
    np.testing.assert_allclose(np.diff(spv._compute_time_steps), dt_resource)


def test_resource_dt_clamps_when_start_equals_file_first_stamp(tmp_path):
    ...
    earlier resource point to include, so the compute grid should start at the
    first resource stamp (i.e. simulation time 0).
    """
    starttime_utc = pd.to_datetime("2024-06-24T17:00:00Z")
...
def test_resource_dt_matches_full_resolution_with_offset_start_within_tolerance(tmp_path):
    """resource path should still match feature-off path with offset start/end.

    This regression guards interval-alignment behavior when simulation start is
    not on the weather file's resource reporting boundary.
    """
    file_start_utc = pd.to_datetime("2024-06-24T17:00:00Z")
    sim_start_utc = pd.to_datetime("2024-06-24T17:00:30Z")
