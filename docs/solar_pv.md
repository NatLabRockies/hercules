...
The solar PV modules use [PySAM](https://nrel-pysam.readthedocs.io/en/main/overview.html) package for the National Renewable Energy Laboratory's System Advisor Model (SAM) to predict the power output of the solar PV plant. 
...
Both solar modules output the AC power (`power`) and the net DC power (`dc_power`) in kW of the PV plant at each timestep, as well as the angle of incidence (`aoi`).
