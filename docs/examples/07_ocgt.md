...
| ~68 | State | 100 MW | → ON (3) | Power reaches P_min (20 MW) after `hot_startup_time` (~8.2 min), turbine now operational |
| ~76 | Ramp | 100 MW | ON (3) | Power reaches 100 MW (ramped at 10 MW/min from P_min) |
| 120 | Command | → 50 MW | ON (3) | Setpoint reduced to 50% capacity |
| ~125 | Ramp | 50 MW | ON (3) | Power reaches 50 MW (ramped down at 10 MW/min) |
| 180 | Command | → 10 MW | ON (3) | Setpoint reduced to 10% (below P_min), power clamped to P_min |
| ~183 | Ramp | 10 MW | ON (3) | Power reaches P_min (20 MW), cannot go lower |
| 210 | Command | → 100 MW | ON (3) | Setpoint increased to full power |
| ~218 | Ramp | 100 MW | ON (3) | Power reaches 100 MW |
| 240 | Command + State | → 0 | → STOPPING (4) | Shutdown command; `min_up_time` satisfied (~172 min on), begins stopping sequence |
